...
const deleteSubCommand = subCommand =>
    subCommand.setName('delete').setDescription('delete all settings.');
...
const deleteSubFunction = async (interaction, userData) => {
    ...
    const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
            ...
            .setCustomId('settings-delete-confirm')
            .setStyle(ButtonStyle.Danger),
        new ButtonBuilder()
            .setLabel(`${lang[langOpt].settings.line_6}`)
            .setCustomId('settings-delete-cancel')
